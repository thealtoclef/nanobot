...
def _handle_models_field(
    working_model: BaseModel, field_name: str, field_display: str, current_value: Any
) -> None:
    """Handle the 'models' list field — add, edit, or remove ModelConfig entries."""
    from nanobot.config.schema import ModelConfig

    while True:
        console.clear()
        _show_section_header(field_display, "Add, edit, or remove models from the fallback chain")

        models_list: list[ModelConfig] = current_value if isinstance(current_value, list) else []
        choices = []
        for i, mc in enumerate(models_list):
            label = f"{mc.name} on {mc.provider}"
            choices.append(f"[{i + 1}] {label}")

        choices.append("[+] Add Model")
        choices.append("<- Back")

        answer = _select_with_back(f"Select model to edit:", choices)
        if answer is _BACK_PRESSED or answer is None or answer == "<- Back":
            break

        if answer == "[+] Add Model":
            new_model = _add_new_model(working_model)
            if new_model is not None:
                models_list.append(new_model)
                current_value = models_list
                setattr(working_model, field_name, models_list)
            continue

        # Parse selection
        for i, choice in enumerate(choices):
            if choice == answer and i < len(models_list):
                selected_model = models_list[i]
                edited = _edit_model_config(selected_model, working_model)
                if edited is not None:
                    if edited is False:
                        # False means remove
                        models_list.pop(i)
                    else:
                        models_list[i] = edited
                    current_value = models_list
                    setattr(working_model, field_name, models_list)
                break


def _add_new_model(working_model: BaseModel) -> "ModelConfig | None":
    """Interactively add a new model to the config."""
    from nanobot.config.schema import ModelConfig

    console.clear()
    _show_section_header("Add New Model", "Configure model name, provider, and settings")

    # Get model name with autocomplete
    model_name = _input_model_with_autocomplete("Model name", "", "auto")
    if not model_name:
        console.print("[yellow]Model name is required[/yellow]")
        return None

    # Auto-fill context window based on model
    _try_auto_fill_context_window(working_model, model_name)

    # Get provider
    provider = _get_current_provider(working_model)
    provider_names = _get_provider_names_for_model_add()
    if not provider_names:
        console.print("[yellow]No providers configured. Please add a provider first.[/yellow]")
        return None

    default_provider = provider if provider != "auto" else (list(provider_names.keys())[0] if provider_names else "")
    selected = _select_with_back("Provider:", list(provider_names.keys()), default=default_provider)
    if selected is _BACK_PRESSED or selected is None:
        return None

    # Map display name back to provider key
    provider_key = provider_names.get(selected, selected)

    # Get temperature
    temp_default = 0.1
    temp_str = _get_questionary().text(
        "Temperature:",
        default=str(temp_default),
    ).ask()
    try:
        temperature = float(temp_str) if temp_str else temp_default
    except ValueError:
        temperature = temp_default

    # Get max_tokens
    tokens_default = 8192
    tokens_str = _get_questionary().text(
        "Max tokens:",
        default=str(tokens_default),
    ).ask()
    try:
        max_tokens = int(tokens_str) if tokens_str else tokens_default
    except ValueError:
...
def _get_provider_names_for_model_add() -> dict[str, str]:
    """Get available providers for model selection (display_name -> provider_key)."""
    ...
    if extra:
        for name in extra:
            if name not in result and not name.startswith('_'):
                result[name.title()] = name
    return result


def _edit_model_config(model: "ModelConfig", working_model: BaseModel) -> "ModelConfig | None | bool":
    """Edit a single ModelConfig. Returns updated model, None for back, or False for remove."""
    from nanobot.config.schema import ModelConfig

    while True:
        console.clear()
        _show_section_header("Edit Model", f"{model.name} on {model.provider}")
        ...
        answer = _select_with_back("Select field to edit:", choices)
        if answer is _BACK_PRESSED or answer is None or answer == "<- Back":
            return None
        if answer == "[Done]":
            return model
        ...
        if answer.startswith("Name: "):
            new_name = _input_model_with_autocomplete("Model name", model.name, model.provider)
            if new_name:
                model.name = new_name
                _try_auto_fill_context_window(working_model, new_name)
        elif answer.startswith("Provider: "):
            provider_names = _get_provider_names_for_model_add()
            if provider_names:
                selected = _select_with_back("Provider:", list(provider_names.keys()), default=model.provider.title())
                if selected and selected not in ("<- Back",):
                    model.provider = provider_names.get(selected, selected)
        elif answer.startswith("Temperature: "):
            temp_str = _get_questionary().text("Temperature:", default=str(model.temperature)).ask()
            if temp_str:
                try:
                    model.temperature = float(temp_str)
                except ValueError:
                    pass
        elif answer.startswith("Max tokens: "):
            tokens_str = _get_questionary().text("Max tokens:", default=str(model.max_tokens)).ask()
            if tokens_str:
                try:
                    model.max_tokens = int(tokens_str)
                except ValueError:
                    pass
...
def _get_provider_names(config: Config) -> dict[str, str]:
    """Get provider display names from config."""
    result = {}
    # Iterate over model_fields to get declared fields
    for name in type(config.providers).model_fields:
        if name.startswith('_'):
            continue
        p = getattr(config.providers, name, None)
    ...
    # Also check __pydantic_extra__ for dynamic (extra="allow") fields
    extra = getattr(config.providers, '__pydantic_extra__', None)
...
def _configure_providers(config: Config) -> None:
    ...
    def get_provider_choices() -> list[str]:
        ...
        choices = []
        for name, display in _get_provider_names(config).items():
...
_SETTINGS_SECTIONS: dict[str, tuple[str, str, set[str] | None]] = {
    "Agent Settings": ("Agent Settings", "Configure models, context window, and behavior", None),
...
_SETTINGS_GETTER = {
    "Agent Settings": lambda c: c.agent,
    "Gateway": lambda c: c.gateway,
    "Tools": lambda c: c.tools,
}

_SETTINGS_SETTER = {
    "Agent Settings": lambda c, v: setattr(c, "agent", v),
...
def _summarize_model(obj: BaseModel) -> list[tuple[str, str]]:
    ...
    for field_name, field_info in type(obj).model_fields.items():
        ...
        if ftype.type_name == "model" and isinstance(value, BaseModel):
            ...
            continue
        if ftype.type_name == "list" and isinstance(value, list):
            # Special handling for models list
            if field_name == "models":
                for i, mc in enumerate(value):
                    if isinstance(mc, BaseModel):
                        name = getattr(mc, "name", "?")
                        provider = getattr(mc, "provider", "?")
                        items.append((f"Model {i + 1}", f"{name} on {provider}"))
                    else:
                        items.append((f"Model {i + 1}", str(mc)))
            else:
                items.append((display, f"{len(value)} item(s)"))
            continue
...
def _show_summary(config: Config) -> None:
    ...
    for name, display in _get_provider_names(config).items():
        provider = getattr(config.providers, name, None)
        status = "[green]configured[/green]" if (provider and provider.get_api_key()) else "[dim]not configured[/dim]"
