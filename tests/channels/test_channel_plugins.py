...
async def test_send_with_retry_skips_send_when_streamed():
    ...
    # BUT if content is non-empty (edge case: streaming requested but no deltas), send anyway
    msg = OutboundMessage(
        channel="streamed", chat_id="123", content="test", metadata={"_streamed": True}
    )
    await mgr._send_with_retry(mgr.channels["streamed"], msg)

    # Content is non-empty, so send is called even with _streamed=True
    ...
    # When _streamed=True but content is empty, send is skipped
    msg_empty = OutboundMessage(
        channel="streamed", chat_id="123", content="", metadata={"_streamed": True}
    )
    await mgr._send_with_retry(mgr.channels["streamed"], msg_empty)
