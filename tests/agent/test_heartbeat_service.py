...
def create_agent(response: str, tmp_path: Path) -> TalkerAgent:
    ...
    return TalkerAgent(workspace=tmp_path, models=[FunctionModel(single_response)])


def create_multi_response_agent(responses: list[str], tmp_path: Path) -> TalkerAgent:
    ...
    return TalkerAgent(workspace=tmp_path, models=[FunctionModel(multi_response)])
...
async def test_decide_prompt_includes_current_time(tmp_path) -> None:
    ...
    agent = TalkerAgent(workspace=tmp_path, models=[FunctionModel(capture_prompt)])
